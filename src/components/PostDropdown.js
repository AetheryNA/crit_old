...
const PostDropdown = () => {
  ...
  return (
    <form
      onSubmit={createPost}
      className="post-modal absolute flex flex-col top-8 right-0 py-2 px-4 w-2/3 rounded-bl-xl"
    >
      <input
        className="mt-5 h-8"
        name="imageData"
      ...
      {contentError ? (
        <p className="post-modal__error mt-2"> {contentError} </p>
      ) : null}

      <textarea
        className="my-5 p-2 h-52"
        name="post-modal-textarea"
