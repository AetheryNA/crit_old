...
const UserProfileBar = ({ loggedUser, currentUser, friends }) => {
  ...
  friends.getFriends.map((friend) => {
    if (currentUser[0].id == friend.friend_id) return (foundFriend = friend.id);
  });
  ...
  return (
    <>
      <div className="user-profile-bar flex flex-row items-center">
        ...
        {loggedUser &&
          (currentUser[0].id === loggedUser.id ? (
            <UserProfileSettings currentUser={loggedUser} />
          ) : foundFriend ? (
            ""
          ) : (
            <UserProfileAddFriend
