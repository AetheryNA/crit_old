...
const PostItem = ({ postItems, user }) => {
  ...
  const postedItem = postItems.map((post, index) => {
    ...
    const addToShareCount = async () => {
      axios.post(`${process.env.BASE_URL}/api/addToReshare`, currentPostData);
    };
    ...
    useEffect(() => {
      const activeState = () => {
        if (
          eachLike[0] &&
          eachLike[0].user_id == user.id &&
          eachLike[0].isLiked == true
        )
          setIsActive(true);
      };
      activeState();
        ...
        <article className="post-item flex flex-col">
          <div className="post-item__profile flex items-center overflow-hidden">
            ...
            <Link href={`/profile/user/${userData.id}`}>
              <a className="ml-3 z-50" href={`/profile/user/${userData.id}`}>
                {" "}
                {userData.username}{" "}
              </a>
            </Link>
            ...
            <div className="post-item__social-status flex justify-between mt-6">
              <div className="post-item__social flex flex-row">
                <div className="post-item__share">
                  <TwitterShareButton
                    title={shareContent}
                    url={`${process.env.BASE_URL}/post/${post.post_id}`}
                    beforeOnClick={addToShareCount}
                  >
                    <TwitterIcon
                      size={30}
                      bgStyle={{ fill: "none" }}
              ...
              <div
                className={`post-item__svg ${isActive ? "active" : ""}`}
                onClick={likeOnClick}
              >
                <Like />
              </div>
