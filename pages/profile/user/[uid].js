  ...
  return (
    <>
      ...
      <div className="profile-pg">
        <UserProfileBar
          currentUser={userPosts && userPosts.users}
          loggedUser={user}
          friends={loggedUserFriends}
        />
        <div className="profile-pg__dashboard flex lg:flex-row flex-col">
          ...
          <div className="dashboard-right">
            <h3 className="flex flex-row">
              <Friends />
              {user &&
                (userPost.id === user.id ? "Your Friends" : "Their Friends")}
            </h3>
...
export const getServerSideProps = withSessionSSR(async function (context) {
  ...
  const [userPostsRes, getFriendsRes, loggedUserFriendsRes] = await Promise.all(
    [
      fetch(
        `http://localhost:3000/api/getUserPosts?user_id=${context.params.uid}`
      ),
      fetch(
        `http://localhost:3000/api/getFriends?user_id=${context.params.uid}`
      ),
